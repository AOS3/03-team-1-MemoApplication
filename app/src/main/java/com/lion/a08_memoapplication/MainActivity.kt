            ...
            navigationViewMain.setNavigationItemSelectedListener {
                when (it.itemId) {
                    R.id.navigation_menu_item_all -> {
                        ...
                        replaceFragment(
                            FragmentName.SHOW_MEMO_ALL_FRAGMENT,
                            false,
                            false,
                            dataBundle
                        )
                    ...
                    R.id.navigation_menu_item_management_category -> {
                        replaceFragment(
                            FragmentName.CATEGORY_MANAGEMENT_FRAGMENT,
                            false,
                            false,
                            null
                        )
                    ...
                    R.id.navigation_menu_item_search_memo -> {
                        val dataBundle = Bundle()
                        dataBundle.putString("MemoName", MemoListName.MEMO_NAME_SEARCH.str)
                        replaceFragment(
                            FragmentName.SEARCH_MEMO_FRAGMENT,
                            false,
                            false,
                            dataBundle
                        )
                        ...
                        dataBundle.putInt(
                            "categoryIdx",
                            categoryList[it.itemId - Menu.FIRST - 1].categoryIdx
                        )
                        dataBundle.putString(
                            "categoryName",
                            categoryList[it.itemId - Menu.FIRST - 1].categoryName
                        )
    ...
    fun addCategoryMenu() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            activityMainBinding.apply {
                ...
                // 하위 메뉴를 추가해준다.
                val subMenu =
                    navigationViewMain.menu.addSubMenu(Menu.NONE, Menu.FIRST, Menu.NONE, "메모 카테고리")

                // 카테고리의 수 만큼 반복한다
                categoryList.forEachIndexed { index, categoryModel ->
                    val subItem = subMenu.add(
                        Menu.NONE,
                        Menu.FIRST + index + 1,
                        Menu.NONE,
